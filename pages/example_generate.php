...
$exampleInsert = $conn->prepare(
    ...
    (dataset_name, accession_id, description, sequence, seq_length, taxon_group, protein_name, display_order)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
);
...
$motifInsert = $conn->prepare(
    "INSERT INTO example_motif_results
    (dataset_name, accession_id, motif_json, residue_scores_json, motif_report_json)
    VALUES (?, ?, ?, ?, ?)"
);
...
foreach ($rows as $row) {
    ...
    $prosite_cmd = "/usr/bin/python3 " . __DIR__ . "/../backend/prosite_scan.py "
                 . escapeshellarg($acc) . " "
...
$exampleResultsInsert = $conn->prepare(
    ...
    (dataset_name, selected_ids, msa_alignment, conservation_scores, msa_report, tree_newick, tree_png_base64, summary_report)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
);
